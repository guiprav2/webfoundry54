...
s.events.on('message:rpc:projects:mv', async ({ ws, rpcid, path, data }) => {
  try {
    console.log('projects.mv:', path);
    ws.send(JSON.stringify({ type: 'rpc:response', rpcid }));
  } catch (err) {
    console.error(err);
...
s.events.on('message:rpc:projects:rm', async ({ ws, rpcid, path, data }) => {
  try {
    console.log('projects.rm:', path);
    ws.send(JSON.stringify({ type: 'rpc:response', rpcid }));
  } catch (err) {
    console.error(err);
    ws.send(JSON.stringify({ type: 'rpc:response', rpcid, error: err.toString() }));
...
s.events.on('message:rpc:files:save', async ({ ws, rpcid, path, data }) => {
  try {
...
s.events.on('message:rpc:files:load', async ({ ws, rpcid, path }) => {
  try {
...
s.events.on('message:rpc:files:mv', async ({ ws, rpcid, path }) => {
  try {
    console.log('files.mv:', path);
    ws.send(JSON.stringify({ type: 'rpc:response', rpcid }));
  } catch (err) {
    console.error(err);
    ws.send(JSON.stringify({ type: 'rpc:response', rpcid, error: err.toString() }));
...
s.events.on('message:rpc:files:rm', async ({ ws, rpcid, path }) => {
  try {
